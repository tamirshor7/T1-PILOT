...
def train_epoch(args, epoch, model, data_loader, optimizer, writer, loader_len):
    ...
    for iter, (sample, tis, target) in enumerate(data_loader):
        ...
        if not args.keep_flip:
            target[:,:3] *= -1 #undo flip
        
        if model.emb_model is not None:
            sample = model.emb_model(tis).permute(0,1,3,2)[...,None] + sample
        
        output = model(sample.permute(0,1,3,2,4),tis).permute(0,1,3,2)
        #output_r = output[:,3:].squeeze()
       
        #A,B,t1 = output[:,0], output[:,1], output[:,2]
        #decay = signal_model(A,B,t1,tis)
        
        rec_loss = criteria(output,target)
    ...
    if epoch and not epoch%10:
        save_comparison_images(output[pi], target[pi], f"{args.exp_dir}/comps_train")
...
def evaluate(args, epoch, model, data_loader, writer, dl_len, train_loss=None, train_rec_loss=None, train_vel_loss=None,
    ...
    pi=0
    if epoch and not epoch%10:
        save_comparison_images(output[pi], target[pi], f"{args.exp_dir}/comps_val")
...
def build_model(args):
    model = Subsampling_Model(
        ...
        multiple_trajectories=not args.single_traj,#args.multi_traj,
        embedding_dim = 0#args.embedding_dim
    ).to(args.device)
    return model
...
def train(args):
    ...
    logging.info(args)
    if args.embedding_dim:
            optimizer.add_param_group({"params": model.emb_model.parameters(), "lr": args.embedding_lr})
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        train_loss, train_time, train_rec_loss, train_vel_loss, train_acc_loss, psnr_train = train_epoch(
            ...
            writer,
            len(train_loader))
        dev_loss, dev_time, psnr_dev = evaluate(args, epoch + 1, model, dev_loader, writer, len(dev_loader),
                                                          train_loss,
...
def create_arg_parser():
    ...
    parser.add_argument('--checkpoint', type=str, default= "/home/tamir.shor/MRI/exps_for_t1_mapping/9_frames_32_shots_bs_12/summary/test/best_model.pt",help='Path to an existing checkpoint. Used along with "--resume"')
    ...
    parser.add_argument('--keep-flip', action='store_true', default=False, help='If true, cancel undo flip')
    parser.add_argument('--single_traj', action='store_true', default=False, help='allow different trajectory per frame if False')
    return parser

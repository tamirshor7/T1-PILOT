...
#     # data = torch.view_as_real(torch.fft.fft(torch.view_as_complex(data), dim=2, norm='ortho'))
#     data = fftshift(data, dim=(-3, -2))
#     return data
...
def ifft2_regular(data):
    ...
    data = torch.view_as_real(torch.fft.ifft2(torch.view_as_complex(data), dim=(2,3), norm='ortho'))
    data = fftshift(data, dim=(-3, -2))
